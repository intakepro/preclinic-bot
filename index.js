...
async function getProfileSession(from) {
  ...
  return { profile_state: data.profile_state || PROFILE_STATE.ENTRY, profile_buffer: data.profile_buffer || {} };
}
...
async function handleProfileModule({ from, body }) {
  ...
  // 全域跳過鍵
  if (input === '0') return { text: '⏭️ 已跳過【基本資料】並直接前往病史模組。', skipToStep: 4 };
  ...
  if (profile_state === PROFILE_STATE.ASK_BIRTHYEAR) {
    ...
    if (!year || year < 1900 || year > 2025) return { text: '格式不正確，請輸入 1900–2025 的 4 位數年份（或輸入 0 跳過）' };
    ...
    return { text: `✅ 已儲存基本資料：\n${renderProfileSummary({ ...existing, ...profile_buffer })}\n\n進入下一步。`, done: true };
  }
