...
// Version: v6.4.5-fs
// 變更重點：
// - 移除 index 端的「第 X 步（製作中）」佔位訊息；完全由模組輸出決定。
// - 若模組沒有回文字，index 不多回任何訊息（HTTP 204）。
// - 其餘行為維持 v6.4.4-fs：z 開始、restart/「我想做預先問診」重設、完成後 step=-1 靜默。
...
async function runStep(stepId, { msg, from }) {
  ...
  try {
    ...
    return { text: `⚠️ 第 ${stepId} 步發生錯誤，請稍後再試或輸入「我想做預先問診」/restart 重設。`, done: false };
  }
}
...
// 健康檢查
app.get('/', (_req, res) => res.send('PreDoctor flow server running. v6.4.5-fs'));
